...
IMPROT_PADDLE_POS=-1   # We will insert "import paddle“ as fourth import node.
...
def count_mdl(node, mdl_name="__future__"):
    assert isinstance(node, gast.gast.Module)
    stat = -1
    count = 0
    for b in node.body: 
        if isinstance(b, gast.gast.ImportFrom): 
            if mdl_name in b.module: 
                stat = 0
                return stat
    return stat
